...
export function greatCircleSailing(
  ...
  const l1 = rad(lat1), l2 = rad(lat2);
  let dLon = rad(lon2 - lon1);
  while (dLon > Math.PI) dLon -= 2 * Math.PI;
  while (dLon < -Math.PI) dLon += 2 * Math.PI;

  // Distance
  const cosD = Math.sin(l1) * Math.sin(l2) + Math.cos(l1) * Math.cos(l2) * Math.cos(dLon);
  const d = Math.acos(Math.max(-1, Math.min(1, cosD)));
  const distNM = deg(d) * 60;

  // Initial course
  const sinC = Math.sin(dLon) * Math.cos(l2);
  const cosC = Math.cos(l1) * Math.sin(l2) - Math.sin(l1) * Math.cos(l2) * Math.cos(dLon);
  let course = deg(Math.atan2(sinC, cosC));
  if (course < 0) course += 360;

  // Vertex (最高緯度)
  const sinLv = Math.sin(l1) / Math.sin(rad(course > 180 ? 360 - course : course) || 1e-10);
...
export function compositeSailing(
  ...
  const cosDLon1 = Math.cos(d1) / (Math.cos(l1) * Math.cos(lv)) - Math.tan(l1) * Math.tan(lv);
  ...
  const cosDLon2 = Math.cos(d2) / (Math.cos(l2) * Math.cos(lv)) - Math.tan(l2) * Math.tan(lv);
  ...
  return {
    totalDistance, initialCourse, gcDist1, gcDist2, parallelDist,
    lonV1, lonV2, isComposite: true,
  };
}
