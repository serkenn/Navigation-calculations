...
import type { GreatCircleResult } from './GreatCircleCalc';

export const GreatCircleResultView = ({ result }: { result: GreatCircleResult }) => (
  <div className="space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
    <div className="relative">
      <h3 className="text-sm font-bold bg-blue-700 text-white inline-block px-3 py-1 mb-3">Great Circle Sailing</h3>
      <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 p-6 text-sm shadow-sm space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex justify-between"><span>最高緯度 (Vertex Lat)</span><span className="font-mono font-bold">{formatDMS(result.latVertex, 'lat')}</span></div>
        </div>
