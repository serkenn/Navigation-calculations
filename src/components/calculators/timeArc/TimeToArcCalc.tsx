...
import { timeToArc } from '../../../utils/timeConversion';

export interface TimeToArcResult {
  deg: number; min: number; sec: number; totalDegrees: number;
  inputH: number; inputM: number; inputS: number;
}

export const TimeToArcCalc = ({ onResult }: { onResult: (r: TimeToArcResult) => void }) => {
  ...
  return (
    <div className="space-y-8 pb-20">
      <header>
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white border-l-4 border-green-600 pl-3">Time → Arc</h2>
        <p className="text-xs text-slate-400 mt-1 pl-4">時間（時分秒）を弧度（度分秒）に変換します</p>
      </header>
