...
import { courseToSteer } from '../../../utils/currentVector';
...
export const CourseToSteerCalc = ({ onResult }: { onResult: (r: CourseToSteerResult) => void }) => {
  const [requiredCMG, setRequiredCMG] = useState(90);
  const [requiredSMG, setRequiredSMG] = useState(12);
  ...
  return (
    <div className="space-y-8 pb-20">
      <header>
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white border-l-4 border-cyan-600 pl-3">視針路・対水速力</h2>
        <p className="text-xs text-slate-400 mt-1 pl-4">目標CMG/SMGと潮流から視針路と対水速力を計算</p>
      </header>
      <section className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4 transition-colors">
        <div className="text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">目標 (Required)</div>
        <div className="grid grid-cols-2 gap-4">
          <NumberInput label="目標CMG" value={requiredCMG} onChange={setRequiredCMG} unit="°" step={0.1} />
          <NumberInput label="目標SMG" value={requiredSMG} onChange={setRequiredSMG} unit="kn" step={0.1} />
        </div>
      </section>
      <section className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4 transition-colors">
