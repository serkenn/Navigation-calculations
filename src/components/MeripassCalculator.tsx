  ...
  <div className="p-6 bg-white rounded-xl shadow-sm border border-slate-100">
    ...
    <div className="space-y-4 text-slate-600 text-sm leading-relaxed">
      ...
      <ol className="list-decimal pl-5 space-y-2">
        ...
        <li>
          <strong>Run to Noon:</strong> 午前観測時から正中時までの針路(Course)と航程(Dist)を入力します。これにより正中時の推測位置($l_0, L_0$)が計算されます。
        </li>
        <li>
          <strong>Noon Sight:</strong> 正中時の観測高度と赤緯を入力します。これにより正中時の実測緯度($l_{obs}$)が計算されます。
        </li>
        <li>
          <strong>計算実行:</strong> ボタンを押すと、経度改正量($\Delta L$)が計算され、最終的な視正午船位が表示されます。
        </li>
      </ol>
    </div>
  </div>
...
const TheoryView = () => (
  <div className="p-6 bg-white rounded-xl shadow-sm border border-slate-100">
    ...
    <div className="space-y-4 text-slate-600 text-sm">
      ...
      <div className="bg-slate-50 p-4 rounded-lg border border-slate-200 my-4 font-mono text-xs md:text-sm">
        <p className="font-bold text-slate-800 mb-2">経度改正公式 ($\Delta L$):</p>
        <p>$$\Delta L = \frac{I \cdot \csc Z - \Delta l \cdot \cot Z}{\cos l_0}$$</p>
        <p className="mt-2 text-slate-500 text-xs">
          プログラム上では以下のように計算しています:<br/>
      ...
      <ul className="list-disc pl-5 space-y-1">
        ...
        <li><strong>$\Delta l$:</strong> Diff between Obs Lat & DR Lat ($l_{obs} - l_0$)</li>
        <li><strong>$l_0$:</strong> DR Latitude at Noon (正中時推測緯度)</li>
      </ul>
    </div>
  </div>
);
...
const MeripassCalculator = () => {
  ...
  const handleCalculate = () => {
    ...
    // 計算したら自動で計算機ビューを表示（スマホ等で見失わないように）
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-gray-50 text-slate-800 font-sans overflow-hidden">
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-64 bg-slate-900 text-white transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 flex flex-col
      ...
      `}>
        ...
        <nav className="flex-1 p-4 space-y-2">
            <button 
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${currentView === 'calculator' ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}
            >
                <Calculator size={18} /> 計算機 (Calculator)
            </button>
            <button 
                onClick={() => changeView('guide')}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${currentView === 'guide' ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}
            >
                <HelpCircle size={18} /> 使い方 (Guide)
            </button>
            <button 
                onClick={() => changeView('theory')}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${currentView === 'theory' ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-slate-800'}`}
            >
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
        ...
        {/* 左パネル: 入力または情報ビュー */}
        <div className="w-full lg:w-1/2 p-6 overflow-y-auto border-r border-gray-200 bg-gray-50 h-full">
                ...
                <div className="space-y-6">
                    ...
                    <section className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div className="flex gap-2">
                                <input type="number" className="w-16 p-2 border border-slate-300 rounded" value={morning.lat.d} onChange={e=>setMorning({...morning, lat:{...morning.lat, d:+e.target.value}})} />
                                <span className="py-2">°</span>
                                <input type="number" className="w-16 p-2 border border-slate-300 rounded" value={morning.lat.m} onChange={e=>setMorning({...morning, lat:{...morning.lat, m:+e.target.value}})} />
                                <span className="py-2">' N</span>
                            </div>
                            </div>
                            <div>
                            <label className="block text-slate-500 mb-1">方位角 (Azimuth)</label>
                            <input type="number" className="w-full p-2 border border-slate-300 rounded" value={morning.azimuth} onChange={e=>setMorning({...morning, azimuth:+e.target.value})} />
                            </div>
                            <div>
                            <label className="block text-slate-500 mb-1">Intercept (I)</label>
                            <input type="number" className="w-full p-2 border border-slate-300 rounded" value={morning.intercept} onChange={e=>setMorning({...morning, intercept:+e.target.value})} />
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <input type="number" className="w-full p-2 border border-slate-300 rounded" value={run.course} onChange={e=>setRun({...run, course:+e.target.value})} />
                            </div>
                            <div>
                            <label className="block text-slate-500 mb-1">航程 (Dist)</label>
                            <input type="number" className="w-full p-2 border border-slate-300 rounded" value={run.distance} onChange={e=>setRun({...run, distance:+e.target.value})} />
                            </div>
                        </div>
                    </section>
                    ...
                    <section className="bg-white p-5 rounded-xl shadow-sm border border-slate-100">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            ...
                            <div className="flex gap-2">
                                <input type="number" className="w-16 p-2 border border-slate-300 rounded" value={noon.obsAlt.d} onChange={e=>setNoon({...noon, obsAlt:{...noon.obsAlt, d:+e.target.value}})} />
                                <span className="py-2">°</span>
                                <input type="number" className="w-16 p-2 border border-slate-300 rounded" value={noon.obsAlt.m} onChange={e=>setNoon({...noon, obsAlt:{...noon.obsAlt, m:+e.target.value}})} />
                                <span className="py-2">'</span>
                            </div>
                            </div>
                            <div>
                            ...
                            <div className="flex gap-2">
                                <input type="number" className="w-16 p-2 border border-slate-300 rounded" value={noon.dec.d} onChange={e=>setNoon({...noon, dec:{...noon.dec, d:+e.target.value}})} />
                                <span className="py-2">°</span>
                                <input type="number" className="w-16 p-2 border border-slate-300 rounded" value={noon.dec.m} onChange={e=>setNoon({...noon, dec:{...noon.dec, m:+e.target.value}})} />
                                <span className="py-2">' N</span>
                            </div>
                            </div>
                        </div>
                    ...
                    <button 
                        ...
                        className="w-full py-4 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition shadow-lg cursor-pointer flex justify-center items-center gap-2"
                    >
        ...
        {/* 右パネル: 計算結果シート */}
        <div className="w-full lg:w-1/2 bg-yellow-50 p-8 border-l border-yellow-200 overflow-y-auto font-mono text-slate-700 relative shadow-inner">
            ...
            <h2 className="text-lg font-bold border-b-2 border-slate-800 pb-2 mb-6 flex items-center justify-between">
                ...
                <span className="text-xs font-normal bg-yellow-200 px-2 py-1 rounded text-yellow-800">Exam Mode</span>
            </h2>
            ...
            <div className="space-y-8 text-sm leading-relaxed animate-in fade-in slide-in-from-bottom-4 duration-500">
                
                {/* 1. Run Calculation */}
                <div className="relative">
                    <div className="absolute -left-6 top-0 text-slate-400 font-sans font-bold opacity-50">01</div>
                ...
                {/* 2. Noon Sight */}
                <div className="relative">
                    <div className="absolute -left-6 top-0 text-slate-400 font-sans font-bold opacity-50">02</div>
                ...
                {/* 3. Meripass Logic */}
                <div className="relative">
                    <div className="absolute -left-6 top-0 text-slate-400 font-sans font-bold opacity-50">03</div>
                    ...
                    <div className="pl-4 border-l-2 border-slate-300 mt-2 space-y-1">
                        ...
                        <div className="my-3 p-3 bg-white border border-slate-200 rounded text-xs">
                            ...
                            <p className="font-serif italic text-center text-sm">
                ...
                <div className="mt-8 p-6 bg-white border-4 border-double border-blue-600 rounded shadow-md relative">
                    <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-blue-600 text-white px-4 py-1 text-xs font-bold uppercase tracking-widest rounded-full">
                        Final Fix
                    </div>
