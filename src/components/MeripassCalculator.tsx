...
import { 
  Calculator, ArrowRight, Sun, Moon, Anchor, FileText, 
...
const GuideView = () => (
  <div className="p-6 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 animate-in fade-in duration-500">
    ...
    <div className="space-y-4 text-slate-600 dark:text-slate-300 text-sm leading-relaxed">
      ...
      <h3 className="font-bold text-slate-800 dark:text-slate-200 border-b dark:border-slate-700 pb-1 mt-4">1. メリパス計算 (3N)</h3>
      ...
      <h3 className="font-bold text-slate-800 dark:text-slate-200 border-b dark:border-slate-700 pb-1 mt-4">2. 出没方位角・ジャイロ誤差</h3>
      <ol className="list-decimal pl-5 space-y-2">
...
const MeripassCalculator = () => {
  ...
  // 競合回避のため、htmlタグのdarkクラスは削除し、このコンポーネント内で完結させる
  useEffect(() => {
  ...
  return (
    <HelmetProvider>
      ...
      {/* ★ Main Wrapper: ここに theme クラスを付与して、配下の全ての dark: クラスを有効化する */}
      <div className={theme}>
        <div className="flex flex-col lg:flex-row h-screen bg-slate-100 dark:bg-slate-950 text-slate-800 dark:text-slate-100 font-sans overflow-hidden transition-colors duration-300">
          ...
          {/* Sidebar Menu */}
          <div className={`fixed inset-0 z-30 bg-black/50 backdrop-blur-sm transition-opacity lg:hidden ${isMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} onClick={() => setIsMenuOpen(false)} />
          
          <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 text-slate-800 dark:text-white transform transition-transform duration-300 lg:relative lg:translate-x-0 flex flex-col shadow-2xl lg:shadow-none ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <main className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
            ...
            <div className={`w-full lg:w-5/12 p-4 md:p-6 overflow-y-auto border-r border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 h-full scrollbar-thin transition-colors ${showMobileResult ? 'hidden lg:block' : 'block'}`}>
              ...
              <div className="space-y-8 pb-20">
                  ...
                  <div className="p-4 space-y-4">
                    <div className="grid grid-cols-2 gap-4 pb-2 border-b border-dashed border-slate-200 dark:border-slate-700">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-1 mt-1">
                                <select className="bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded p-1 text-sm text-slate-900 dark:text-white outline-none" value={noon.eqTime.sign} onChange={e => setNoon({...noon, eqTime: {...noon.eqTime, sign: +e.target.value}})}><option value={1} className="bg-white dark:bg-slate-800">+</option><option value={-1} className="bg-white dark:bg-slate-800">-</option></select>
                    ...
                    <div className="grid grid-cols-2 gap-4 items-end">
                      ...
                      <div>
                          <label className="text-[10px] text-slate-500 dark:text-slate-400 font-bold block">改正総数 (Corr)</label>
                          <div className="flex items-center gap-1 mt-1">
                            <select className="bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded p-1.5 text-sm font-bold text-slate-900 dark:text-white outline-none" value={noon.totalCorrSign} onChange={e => setNoon({...noon, totalCorrSign: +e.target.value})}><option value={1} className="bg-white dark:bg-slate-800">+</option><option value={-1} className="bg-white dark:bg-slate-800">-</option></select>
                ...
                <button onClick={handleCalculateMeripass} className="w-full py-4 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 flex justify-center items-center gap-2">
              ...
              <div className="space-y-8 pb-20">
                ...
                <section className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-6 transition-colors">
                  ...
                  <button onClick={handleCalculateGyro} className="w-full py-4 bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-500 hover:to-emerald-600 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 flex justify-center items-center gap-2">
            ...
            {/* --- Right Panel (Output) --- */}
            {/* ★ Mobile result logic: showMobileResult ? full screen : hidden on mobile */}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                ...
                                <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 p-4 text-sm space-y-1 shadow-sm">
                                    ...
                                    <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400"><span>(Total Corr)</span> <span>{morning.totalCorrSign>=0?'+':'-'}{morning.totalCorr}'</span></div>
                            ...
                            <div>
                                ...
                                <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 p-4 text-sm space-y-1 shadow-sm">
                                    ...
                                    <div className="flex justify-between text-xs text-slate-500 dark:text-slate-400"><span>(Corr)</span> <span>{noon.totalCorrSign>=0?'+':'-'}{noon.totalCorr}'</span></div>
                          ...
                          <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 p-6 text-sm shadow-sm space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-8 border-b border-slate-200 dark:border-slate-800 pb-4">
                               <div>
                                 ...
                                 <p className="text-xl font-bold text-slate-800 dark:text-white">{gyroResult.direction}</p>
                               </div>
                            ...
                            <div>
                               <div className="flex justify-between items-center mb-2">
                                 <span className="font-bold text-slate-600 dark:text-slate-300">True Azimuth (360°)</span>
